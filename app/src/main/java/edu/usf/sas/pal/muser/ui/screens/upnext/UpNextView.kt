package edu.usf.sas.pal.muser.ui.screens.upnext
...
import edu.usf.sas.pal.muser.glide.palette.ColorSet
...
class UpNextView constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : LinearLayout(context, attrs, defStyleAttr) {
    ...
    private lateinit var settingsManager: edu.usf.sas.pal.muser.utils.SettingsManager
    ...
    init {
        ...
        playerViewAdapter = object : PlayerViewAdapter() {
            ...
            override fun shuffleChanged(shuffleMode: Int) {
                (shuffleButton as? edu.usf.sas.pal.muser.ui.views.ShuffleButton)?.setShuffleMode(shuffleMode)
            }

            override fun repeatChanged(repeatMode: Int) {
                (repeatButton as? edu.usf.sas.pal.muser.ui.views.RepeatButton)?.setRepeatMode(repeatMode)
            }
            ...
            override fun trackInfoChanged(song: edu.usf.sas.pal.muser.model.Song?) {
                ...
                if (isLandscape && settingsManager.usePalette) {
                    ...
                    Glide.with(getContext())
                        .load(song)
                        .asBitmap()
                        .transcode(edu.usf.sas.pal.muser.glide.palette.ColorSetTranscoder(getContext()), ColorSet::class.java)
                        .override(250, 250)
    ...
    override fun onAttachedToWindow() {
        ...
        isLandscape = edu.usf.sas.pal.muser.utils.ShuttleUtils.isLandscape(context.applicationContext as edu.usf.sas.pal.muser.ShuttleApplication)
        ...
        if (!isLandscape) {
            ...
            disposables.add(
                Observable.combineLatest(
                    edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorPrimary(),
                    edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorPrimaryInverse(),
                    Observable.just(false),
                    edu.usf.sas.afollestad.aesthetic.LightDarkColorState.creator()
                )
        ...
        disposables.add(edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).isDark
            .map { isDark -> isLandscape && !isDark }
            .flatMap { isDark -> if (isDark) edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorPrimaryInverse() else edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorPrimary() }
            .subscribe { color -> queueTextView.setTextColor(color) })

        disposables.add(edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).isDark
            .map { isDark -> isLandscape && !isDark }
            .flatMap { isDark -> if (isDark) edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorSecondaryInverse() else edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).textColorSecondary() }
                ...
                observable.subscribe(
                    { colorSet ->
                        if (this@UpNextView.colorSet == colorSet) {
                            ...
                            animateColors(this@UpNextView.colorSet, colorSet, 800, edu.usf.sas.pal.muser.rx.UnsafeConsumer { this.invalidateColors(it) }, null)
                        }
                    },
                    { _ ->
                        // Nothing to do
        ...
        seekBar?.let { seekBar ->
            ...
            }, { error -> edu.usf.sas.pal.muser.utils.LogUtils.logException(TAG, "Error in seek change event", error) }))
            ...
            disposables.add(
                sharedSeekBarEvents
                    ...
                    .subscribe({ seekBarChangeEvent -> playerPresenter.seekTo(seekBarChangeEvent.progress()) },
                        { error -> edu.usf.sas.pal.muser.utils.LogUtils.logException(TAG, "Error receiving seekbar progress", error) })
            )
        }
    }
    ...
    fun invalidateColors(colorSet: ColorSet) {
        ...
        if (isLandscape) {
            ...
            (shuffleButton as? edu.usf.sas.pal.muser.ui.views.ShuffleButton)?.invalidateColors(colorSet.primaryTextColor, colorSet.primaryTextColorTinted)

            (repeatButton as? edu.usf.sas.pal.muser.ui.views.RepeatButton)?.invalidateColors(colorSet.primaryTextColor, colorSet.primaryTextColorTinted)
            ...
            seekBar?.invalidateColors(edu.usf.sas.afollestad.aesthetic.ColorIsDarkState(if (ignorePalette) colorSet.accentColor else colorSet.primaryTextColorTinted, false))
    ...
    private fun animateColors(from: ColorSet, to: ColorSet, duration: Int, consumer: edu.usf.sas.pal.muser.rx.UnsafeConsumer<ColorSet>, onComplete: edu.usf.sas.pal.muser.rx.UnsafeAction?) {
        ...
        val argbEvaluator = edu.usf.sas.pal.muser.utils.color.ArgbEvaluator.getInstance()
    ...
    private fun getAestheticColorSetDisposable(): Observable<ColorSet> {
        return Observable.combineLatest(
            edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).colorPrimary(),
            edu.usf.sas.afollestad.aesthetic.Aesthetic.get(context).colorAccent(),
    ...
    private val paletteTarget = object : SimpleTarget<ColorSet>() {
        override fun onResourceReady(newColorSet: ColorSet, glideAnimation: GlideAnimation<in ColorSet>) {
            ...
            animateColors(
                ...
                newColorSet,
                800,
                    edu.usf.sas.pal.muser.rx.UnsafeConsumer { intermediateColorSet ->
                        ...
                        if (settingsManager.tintNavBar) {
                            edu.usf.sas.afollestad.aesthetic.Aesthetic.get(getContext()).colorNavigationBar(intermediateColorSet.primaryColor).apply()
                        }
                    },
                    edu.usf.sas.pal.muser.rx.UnsafeAction {
                        ...
                        if (!settingsManager.usePaletteNowPlayingOnly) {
                            ...
                            animateColors(oldColorSet, newColorSet, 450, edu.usf.sas.pal.muser.rx.UnsafeConsumer { intermediateColorSet ->
                                val aesthetic = edu.usf.sas.afollestad.aesthetic.Aesthetic.get(getContext())
        ...
        override fun onLoadFailed(e: Exception?, errorDrawable: Drawable?) {
            ...
            getAestheticColorSetDisposable()
                .take(1)
                .subscribe(
                    { colorSet -> animateColors(this@UpNextView.colorSet, colorSet, 800, edu.usf.sas.pal.muser.rx.UnsafeConsumer { intermediateColorSet -> invalidateColors(intermediateColorSet) }, null) },
                    { _ ->
                        // Nothing ot do
                    }
                )
    ...
    companion object {
        ...
        fun newInstance(context: Context, playerPresenter: PlayerPresenter, settingsManager: edu.usf.sas.pal.muser.utils.SettingsManager): UpNextView {
