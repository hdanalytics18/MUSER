...
import edu.usf.sas.pal.muser.utils.BaseSettingsManager
...
class PlaybackSettingsManager @Inject constructor(sharedPreferences: SharedPreferences) : BaseSettingsManager(sharedPreferences) {
    ...
    @edu.usf.sas.pal.muser.playback.QueueManager.RepeatMode()
    var repeatMode: Int
        get() = getInt(KEY_REPEAT_MODE, edu.usf.sas.pal.muser.playback.QueueManager.RepeatMode.OFF)
        set(repeatMode) = setInt(KEY_REPEAT_MODE, repeatMode)

    private val KEY_SHUFFLE_MODE = "shuffle_mode"
    @edu.usf.sas.pal.muser.playback.QueueManager.ShuffleMode()
    var shuffleMode: Int
        get() = getInt(KEY_SHUFFLE_MODE, edu.usf.sas.pal.muser.playback.QueueManager.ShuffleMode.OFF)
