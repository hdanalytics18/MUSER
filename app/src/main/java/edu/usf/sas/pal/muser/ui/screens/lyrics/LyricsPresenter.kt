...
constructor(
...
) : edu.usf.sas.pal.muser.ui.common.Presenter<LyricsView>() {
    ...
    override fun bindView(view: LyricsView) {
        ...
        addDisposable(
            RxBroadcast.fromBroadcast(application, IntentFilter(edu.usf.sas.pal.muser.playback.constants.InternalIntents.META_CHANGED))
                .toFlowable(BackpressureStrategy.LATEST)
                .subscribe(
                    { _ -> updateLyrics(application) },
                    { error -> edu.usf.sas.pal.muser.utils.LogUtils.logException(TAG, "Error receiving meta changed", error) }
                )
        )
    }
    ...
    fun downloadOrLaunchQuickLyric() {
        val lyricsView = view
        if (lyricsView != null) {
            if (edu.usf.sas.pal.muser.ui.screens.lyrics.QuickLyricUtils.isQLInstalled(application)) {
        ...
        addDisposable(
            Observable.fromCallable(Callable {
                ...
                if (path!!.startsWith("content://")) {
                    ...
                    val cursor = edu.usf.sas.pal.muser.sql.SqlUtils.createQuery(application, query)
                    if (cursor != null) {
                        try {
                ...
                .subscribe(
                    { lyrics ->
                        ...
                        if (lyricsView != null) {
                            ...
                            lyricsView.showQuickLyricInfoButton(!edu.usf.sas.pal.muser.ui.screens.lyrics.QuickLyricUtils.isQLInstalled(context))
                        }
                    },
                    { error -> edu.usf.sas.pal.muser.utils.LogUtils.logException(TAG, "Error getting lyrics", error) })
        )
    }
